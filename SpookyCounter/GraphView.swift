...
let GraphBackgroundColor = UIColor(hex: 0x220055)
let GraphOrangeColor = UIColor(red: 255/255, green: 128/255, blue: 0/255, alpha: 1)
...
class GraphView: UIView {
    ...
    var showLines   = true
    var showPoints  = true
    var linesColor  = GraphOrangeColor
    var graphColor  = GraphOrangeColor
    var labelFont   = UIFont.systemFont(ofSize: 12)
    var labelColor  = GraphOrangeColor
    var xAxisColor  = GraphOrangeColor
    var yAxisColor  = GraphOrangeColor
    ...
    override func draw(_ rect: CGRect) {
        ...
        for point in data {
            let n = ((point as AnyObject).object(forKey: "value") as! NSNumber).intValue
    ...
    func plotPoint(point : NSDictionary, path: CGMutablePath) {
        ...
        var interval = 1
        if data.count > 1 {
            interval = Int(graphWidth - xMargin * 2) / (data.count - 1);
        }        
        ...
        xLabel.frame = CGRect(x: xposition - 17, y: graphHeight + 12, width: 36, height: 20)
